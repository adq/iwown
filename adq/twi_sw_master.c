...
#define TWI_SCL_HIGH(CONFIG) do { NRF_GPIO->OUTSET = (1UL << CONFIG.TWI_Pin_SCL); } while(0)   /*!< Pulls SCL line high */
#define TWI_SCL_LOW(CONFIG) do { NRF_GPIO->OUTCLR = (1UL << CONFIG.TWI_Pin_SCL); } while(0)  /*!< Pulls SCL line low  */
#define TWI_SDA_HIGH(CONFIG) do { NRF_GPIO->OUTSET = (1UL << CONFIG.TWI_Pin_SDA);  } while(0)  /*!< Pulls SDA line high */
#define TWI_SDA_LOW(CONFIG) do { NRF_GPIO->OUTCLR = (1UL << CONFIG.TWI_Pin_SDA);  } while(0)   /*!< Pulls SDA line low  */
#define TWI_SDA_INPUT(CONFIG) do { NRF_GPIO->DIRCLR = (1UL << CONFIG.TWI_Pin_SDA);  } while(0)   /*!< Configures SDA pin as input  */
#define TWI_SDA_OUTPUT(CONFIG) do { NRF_GPIO->DIRSET = (1UL << CONFIG.TWI_Pin_SDA);  } while(0)   /*!< Configures SDA pin as output */
#define TWI_SCL_OUTPUT(CONFIG) do { NRF_GPIO->DIRSET = (1UL << CONFIG.TWI_Pin_SCL); } while(0)   /*!< Configures SCL pin as output */
#define TWI_SDA_READ(CONFIG)  ((NRF_GPIO->IN >> CONFIG.TWI_Pin_SDA) & 0x1UL)                     /*!< Reads current state of SDA */
#define TWI_SCL_READ(CONFIG) ((NRF_GPIO->IN >> CONFIG.TWI_Pin_SCL) & 0x1UL)                    /*!< Reads current state of SCL */

/*lint -e415 -e845 -save "Out of bounds access" */
#define TWI_SDA_STANDARD0_NODRIVE1(CONFIG) do { \
        NRF_GPIO->PIN_CNF[CONFIG.TWI_Pin_SDA] = (GPIO_PIN_CNF_SENSE_Disabled << GPIO_PIN_CNF_SENSE_Pos) \
...
#define TWI_SCL_STANDARD0_NODRIVE1(CONFIG) do { \
        NRF_GPIO->PIN_CNF[CONFIG.TWI_Pin_SCL] = (GPIO_PIN_CNF_SENSE_Disabled << GPIO_PIN_CNF_SENSE_Pos) \
        |(GPIO_PIN_CNF_DRIVE_S0D1 << GPIO_PIN_CNF_DRIVE_Pos)    \
        |(GPIO_PIN_CNF_PULL_Pullup << GPIO_PIN_CNF_PULL_Pos)  \
        |(GPIO_PIN_CNF_INPUT_Connect << GPIO_PIN_CNF_INPUT_Pos) \
        |(GPIO_PIN_CNF_DIR_Input << GPIO_PIN_CNF_DIR_Pos);  \
...
bool twi_master_init(twi_master_config_t twi_master_config)
{
    ...
    TWI_SDA_STANDARD0_NODRIVE1(twi_master_config); /*lint !e416 "Creation of out of bounds pointer" */
    TWI_SCL_STANDARD0_NODRIVE1(twi_master_config); /*lint !e416 "Creation of out of bounds pointer" */

    // Configure SCL as output
    TWI_SCL_HIGH(twi_master_config);
    TWI_SCL_OUTPUT(twi_master_config);

    // Configure SDA as output
    TWI_SDA_HIGH(twi_master_config);
    TWI_SDA_OUTPUT(twi_master_config);

    return twi_master_clear_bus(twi_master_config);
}
...
bool twi_master_transfer(twi_master_config_t twi_master_config, uint8_t address, uint8_t * data, uint8_t data_length, bool issue_stop_condition)
{
    ...
    transfer_succeeded &= twi_master_issue_startcondition(twi_master_config);
    transfer_succeeded &= twi_master_clock_byte(twi_master_config, address);
    ...
    if (address & TWI_READ_BIT)
    {
        ...
        while (data_length-- && transfer_succeeded)
        {
            ...
            if (data_length == 0)
            {
                transfer_succeeded &= twi_master_clock_byte_in(twi_master_config, data, (bool)false);
            }
            else
            {
                transfer_succeeded &= twi_master_clock_byte_in(twi_master_config, data, (bool)true);
            }
            data++;
        }
    }
    else
    {
        ...
        while (data_length-- && transfer_succeeded)
        {
            transfer_succeeded &= twi_master_clock_byte(twi_master_config, *data);
            data++;
        }
    }
    ...
    {
        transfer_succeeded &= twi_master_issue_stopcondition(twi_master_config);
    }
...
static bool twi_master_clear_bus(twi_master_config_t twi_master_config)
{
    ...
    if (TWI_SDA_READ(twi_master_config) == 1 && TWI_SCL_READ(twi_master_config) == 1)
    {
        bus_clear = true;
    }
    else if (TWI_SCL_READ(twi_master_config) == 1)
    {
        bus_clear = false;
        ...
        {
            TWI_SCL_LOW(twi_master_config);
            TWI_DELAY();
            TWI_SCL_HIGH(twi_master_config);
            TWI_DELAY();

            if (TWI_SDA_READ(twi_master_config) == 1)
            {
                bus_clear = true;
                break;
            }
...
bool twi_master_issue_startcondition(twi_master_config_t twi_master_config)
{
    ...
    {
        // Pull SDA low
        TWI_SDA_LOW(twi_master_config);
    }
    else if (TWI_SCL_READ(twi_master_config) == 1 && TWI_SDA_READ(twi_master_config) == 0)
    {
        // Issue Stop by pulling SDA high
        TWI_SDA_HIGH(twi_master_config);
        TWI_DELAY(twi_master_config);

        // Then Start by pulling SDA low
        TWI_SDA_LOW(twi_master_config);
    }
    else if (TWI_SCL_READ(twi_master_config) == 0 && TWI_SDA_READ(twi_master_config) == 0)
    {
        // First pull SDA high
        TWI_SDA_HIGH(twi_master_config);

        // Then SCL high
        if (!twi_master_wait_while_scl_low(twi_master_config))
        {
            return false;
        }
    ...
    else if (TWI_SCL_READ(twi_master_config) == 0 && TWI_SDA_READ(twi_master_config) == 1)
    {
        // SCL high
        if (!twi_master_wait_while_scl_low(twi_master_config))
        {
...
bool twi_master_clock_byte(twi_master_config_t twi_master_config, uint_fast8_t databyte)
{
    ...
    TWI_SDA_INPUT(twi_master_config);
    ...
    transfer_succeeded &= twi_master_wait_while_scl_low(twi_master_config);

    // Read ACK/NACK. NACK == 1, ACK == 0
    transfer_succeeded &= !(TWI_SDA_READ(twi_master_config));
...
bool twi_master_clock_byte_in(twi_master_config_t twi_master_config, uint8_t *databyte, bool ack)
{
    ...
    for (uint_fast8_t i = 0x80; i != 0; i >>= 1)
    {
        ...
        if (TWI_SDA_READ(twi_master_config))
        {
            byte_read |= i;
        }
        else
...
static bool twi_master_wait_while_scl_low(twi_master_config_t twi_master_config)
{
    ...
    while (TWI_SCL_READ(twi_master_config) == 0)
    {
